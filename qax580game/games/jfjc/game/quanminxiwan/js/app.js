function initStartScreen() {
	...
	userInput.addHitArea("moreGames", butEventHandler, null, "image", t);
	...
	panel = new Elements.Panel(assetLib.getData("panels"), assetLib.getData("bigNumbers"), assetLib.getData("smallNumbers"), gameState, r, canvas.width, canvas.height),
	panel.startTween(),
	aBubbles = new Array;
	for (var i = 0; 20 > i; i++) {
...
function initPreGame() {
	gameState = "tutorial",
	background = new Elements.Background(assetLib.getData("background"), canvas.width, canvas.height);
	for (var e = 0; e < aTutorials.length; e++) if (aTutorials[e].levelNum == levelNum && !aTutorials[e].shown) {
		var t = {
			oImgData: assetLib.getData("uiButs"),
		...
		panel = new Elements.Panel(assetLib.getData("panels"), assetLib.getData("bigNumbers"), assetLib.getData("smallNumbers"), aTutorials[e].panelType, r, canvas.width, canvas.height),
		panelFrame = aTutorials[e].panelFrame,
		panel.startTween(),
		previousTime = (new Date).getTime(),
		updateTutorialEvent(),
		void 0
...
function initGame() {
	gameState = "game",
	1 == audioType && music.volume(.5),
	background = new Elements.Background(assetLib.getData("background"), canvas.width, canvas.height),
	userInput.addHitArea("pause", butEventHandler, null, "rect", {
		aRect: [0, 0, 53, 53]
	},
	!0),
	userInput.addHitArea("wash", butEventHandler, {
		isDraggable: !0,
		multiTouch: !0
	},
	"rect", {
	...
	userInput.addHitArea("logo", butEventHandler, null, "image", oSpilLogoBut),
	water = new Elements.Water(assetLib.getData("water"), canvas.width, canvas.height),
	hud = new Elements.Hud(assetLib.getData("hud"), assetLib.getData("bigNumbers"), assetLib.getData("smallNumbers"), canvas.width, canvas.height),
	gameTimer = 0,
	allowWashing = !0,
	addCrockery(),
	aBubbles = new Array,
...
function dsdsff() {
	...
	if (e.show && e.action) {
		...
		t.addEventListener("click", e.action),
		t.classList.remove("spilgames-splash-screen-gone"),
		window.setTimeout(function() {
			t.classList.add("spilgames-splash-screen-gone"),
			1 != audioType || muted || music.play(),
			initStartScreen(),
			resizeCanvas()
		},
		2e3)
	} else 1 != audioType || muted || music.play(),
	initStartScreen(),
	resizeCanvas()
}
function butEventHandler(e, t) {
	...
	case "startGame":
		...
		userInput.removeHitArea("moreGames"),
		userInput.removeHitArea("logo"),
		initPreGame();
		break;
	case "logo":
		playSound("click"),
		oSpilLogoData.action();
		break;
	case "moreGames":
		playSound("click"),
		clickMore();
	...
	case "replay":
		...
		levelScore = 0,
		initGame(),
		spilAPI.GameBreak.request(adPauseOn, adPauseOff),
		console.log("show ad");
		break;
	case "exitFromEnd":
		playSound("click"),
...
function initGameOver() {
	...
	userInput.addHitArea("moreGames", butEventHandler, null, "image", e),
	userInput.addHitArea("replay", butEventHandler, null, "image", t),
	userInput.addHitArea("exitFromEnd", butEventHandler, null, "image", n);
	...
	panel = new Elements.Panel(assetLib.getData("panels"), assetLib.getData("bigNumbers"), assetLib.getData("smallNumbers"), gameState, i, canvas.width, canvas.height),
	panel.startTween(),
	aBubbles = new Array;
	for (var s = 0; 20 > s; s++) {
	...
	levelScore += 100 * hud.washedNum,
	myPlayGameScore = levelScore;
	levelScore > highScore && (highScore = levelScore),
	panel.oScoreData = {
	...
	previousTime = (new Date).getTime(),
	updateGameOver();
	dp_submitScore(-1,myPlayGameScore);
	dp_share(myPlayGameScore);
}
function addCrockery() {
	playSound("startSplash"),
	crockery = new Elements.Crockery(assetLib.getData("crockery"), assetLib.getData("expressions"), assetLib.getData("dirt"), crockeryCallback, crockeryId, canvas.width, canvas.height),
	crockery.startTween(),
	crockeryId = (crockeryId + 1) % 5
}
function addBubble(e) {
	var t;
	flipFlop ? (playSound("wash" + Math.floor(4 * Math.random())), flipFlop = !1) : flipFlop = !0,
	t = new Elements.Bubble(assetLib.getData("bubbles"), Math.floor(4 * Math.random()), aCurWashPos, canvas.width, canvas.height);
	var n = aPrevWashPos[0] - aCurWashPos[0],
	r = aPrevWashPos[1] - aCurWashPos[1],
	i = (n * n + r * r) / e / 2e3;
	aBubbles.push(t),
	crockery.wash(Math.min(i, .5), aCurWashPos)
}
function addSparkle() {
	var e;
	e = new Elements.Bubble(assetLib.getData("bubbles"), 4, [400 * Math.random() + 25, 300 * Math.random() + 150], canvas.width, canvas.height),
	aBubbles.push(e)
}
...
function updateGameEvent() {
	if (!manualPause && !rotatePause && "game" == gameState) {
		var e = getDelta();
		if (gameTimer += e, gameTimer >= 1 && (gameTimer = 0, --hud.time <= 0)) return initGameOver(),
		void 0;
		...
		crockery.render(ctx),
		bubbleLimiter += e,
		isWashing && bubbleLimiter > .15 && aBubbles.length < 20 && (addBubble(e), isWashing = !1, bubbleLimiter = 0);
		for (var t = 0; t < aBubbles.length; t++) aBubbles[t].update(e),
		renderSprite(aBubbles[t]),
		aBubbles[t].removeMe && (aBubbles.splice(t, 1), t -= 1);
		hud.render(ctx),
		renderMuteBut(),
		renderLogoBut(),
...
function updateGameOver() {
	if (!rotatePause && "gameOver" == gameState) {
		...
		for (var t = 0; t < aBubbles.length; t++) aBubbles[t].update(e),
		renderSprite(aBubbles[t]),
		aBubbles[t].removeMe && (aBubbles[t].x = 450 * Math.round(1 * Math.random()), aBubbles[t].y = 600 * Math.random(), aBubbles[t].vy = 0, aBubbles[t].startTween(), aBubbles[t].removeMe = !1);
		renderMuteBut(),
...
function updateSplashScreenEvent() {
	if (!rotatePause && "splash" == gameState) {
		var e = getDelta();
		if (splashTimer += e, splashTimer > 2.5) return 1 != audioType || muted || music.play(),
		initStartScreen(),
		void 0;
		splash.render(ctx, e),
...
function getCentreFromTopLeft(e, t, n) {
	var r = new Array;
	return r.push(e[0] + t.oData.spriteWidth / 2 * n),
	r.push(e[1] + t.oData.spriteHeight / 2 * n),
	r
}
...
function loadAssets() {
	...
	{
		id: "logo",
		file: "images/logo_GGG_202x50.png"
...
function toggleManualPause() {
	...
	pauseCoreOff();
		...
		userInput.addHitArea("moreGames", butEventHandler, null, "image", n),
		panel = new Elements.Panel(assetLib.getData("panels"), assetLib.getData("bigNumbers"), assetLib.getData("smallNumbers"), "pause", r, canvas.width, canvas.height),
		panel.render(ctx),
		renderLogoBut(),
		userInput.addHitArea("pause", butEventHandler, null, "rect", {
			aRect: [0, 0, 53, 53]
...
function(e) {
	var t = function() {
		function e(e, t, n, r, i, s) {
			...
			this.topLeftY = 370,
			this.showBar && (ctx.strokeStyle = "#ffffff", ctx.lineWidth = 2, ctx.fillStyle = "#D27B5A", ctx.moveTo(this.topLeftX, this.topLeftY), ctx.lineTo(this.topLeftX + r / 2, this.topLeftY + 0), ctx.lineTo(this.topLeftX + r / 2, this.topLeftY + 20), ctx.lineTo(this.topLeftX + 0, this.topLeftY + 20), ctx.lineTo(this.topLeftX + 0, this.topLeftY + 0), ctx.stroke());
		...
		return e.prototype.loadImage = function(e) {
			...
			n.onload = function() {
				...
				void 0 != e.oData ? (t.oAssetData[e.id].oData = e.oData, t.oAssetData[e.id].isSpriteSheet = !0) : (t.oAssetData[e.id].isSpriteSheet = !1, t.oAssetData[e.id].oData = {},
				t.oAssetData[e.id].oData.spriteWidth = t.oAssetData[e.id].img.width, t.oAssetData[e.id].oData.spriteHeight = t.oAssetData[e.id].img.height),
				++t.assetsLoaded,
				t.showBar && ctx.fillRect(t.topLeftX + 2, t.topLeftY + 2, (t.canvasWidth / 2 - 4) / t.totalAssets * t.assetsLoaded, 16),
				t.checkLoadComplete();
		...
		e.prototype.loadedCallback = function() {
...
function(e) {
	var t = function() {
		function e(e, t) {
			...
			function(e) {
				n.isDown = !0,
				n.hitDown(e, e.pageX, e.pageY, 1)
			},
			!1),
			e.addEventListener("mouseup",
			function(e) {
				n.isDown = !1,
				n.hitUp(e, e.pageX, e.pageY, 1)
			},
			!1),
		...
		e.prototype.hitDown = function(e, t, n, r) {
			if (!this.pauseIsOn) {
				...
				if (! (i - this.prevHitTime < 500 && isBugBrowser)) {
					...
					for (var s = 0; s < this.aHitAreas.length; s++) if (this.aHitAreas[s].rect && t > this.aHitAreas[s].area[0] && n > this.aHitAreas[s].area[1] && t < this.aHitAreas[s].area[2] && n < this.aHitAreas[s].area[3]) {
						this.aHitAreas[s].aTouchIdentifiers.push(r),
						this.aHitAreas[s].oData.isDown || (this.aHitAreas[s].oData.isDown = !0, this.aHitAreas[s].oData.x = t, this.aHitAreas[s].oData.y = n, this.aHitAreas[s].callback(this.aHitAreas[s].id, this.aHitAreas[s].oData));
						break
					}
				}
			}
		...
		e.prototype.hitUp = function(e, t, n, r) {
			if (!this.pauseIsOn) {
				...
				t = (t - this.canvasX) * this.canvasScaleX,
				n = (n - this.canvasY) * this.canvasScaleY;
				for (var i = 0; i < this.aHitAreas.length; i++) if (this.aHitAreas[i].rect && t > this.aHitAreas[i].area[0] && n > this.aHitAreas[i].area[1] && t < this.aHitAreas[i].area[2] && n < this.aHitAreas[i].area[3]) {
		...
		e.prototype.move = function(e, t, n, r, i) {
			if (!this.pauseIsOn && i) {
				t = (t - this.canvasX) * this.canvasScaleX,
				n = (n - this.canvasY) * this.canvasScaleY;
				for (var s = 0; s < this.aHitAreas.length; s++) if (this.aHitAreas[s].rect) if (t > this.aHitAreas[s].area[0] && n > this.aHitAreas[s].area[1] && t < this.aHitAreas[s].area[2] && n < this.aHitAreas[s].area[3]) this.aHitAreas[s].oData.isDown || (this.aHitAreas[s].oData.isDown = !0, this.aHitAreas[s].oData.x = t, this.aHitAreas[s].oData.y = n, this.aHitAreas[s].aTouchIdentifiers.push(r), this.aHitAreas[s].oData.multiTouch && this.aHitAreas[s].callback(this.aHitAreas[s].id, this.aHitAreas[s].oData)),
				this.aHitAreas[s].oData.isDraggable && (this.aHitAreas[s].oData.isBeingDragged = !0, this.aHitAreas[s].oData.x = t, this.aHitAreas[s].oData.y = n, this.aHitAreas[s].callback(this.aHitAreas[s].id, this.aHitAreas[s].oData), this.aHitAreas[s].oData.isBeingDragged = !1);
	...
	var t = function() {
		function e(e) {
			...
			this.render = function(e) {
				...
				e.fill(),
				e.fillStyle = "#ffffff",
				e.fillText(Math.round(1e3 / (1e3 * this.display)) + " fps " + this.log, 5, this.canvasHeight - 5)
			},
			this.canvasHeight = e
		}
...
function(e) {
	var t = function() {
		...
		e.prototype.renderScroll = function(e) {
			...
			e.drawImage(this.oImgData.img, this.x, this.y, this.canvasWidth, this.canvasHeight, 0, 0, this.canvasWidth, this.canvasHeight)
		},
		e.prototype.render = function(e) {
...
function(e) {
	var t = function() {
		...
		return e.prototype.render = function(e, t) {
			this.inc += 5 * t,
			e.drawImage(this.oSplashScreenImgData.img, 0, 0 - this.posY)
		},
		e
	} ();
	e.Splash = t
...
function(e) {
	var t = function() {
		function e(e, t, n, r, i, s, o) {
			...
			this.endTime = 0,
			this.posY = 0,
			this.bigCharSpace = 42,
			this.smallCharSpace = 20,
			this.incY = 0,
			this.oPanelsImgData = e,
			this.oBigNumbersImgData = t,
			this.oSmallNumbersImgData = n,
			this.panelType = r,
			this.aButs = i,
			this.canvasWidth = s,
			this.canvasHeight = o
		...
		e.prototype.render = function(e) {
			...
			case "gameOver":
				...
				r = Math.floor(t / (this.oPanelsImgData.img.width / this.oPanelsImgData.oData.spriteWidth)) * this.oPanelsImgData.oData.spriteHeight;
				e.drawImage(this.oPanelsImgData.img, n, r, this.oPanelsImgData.oData.spriteWidth, this.oPanelsImgData.oData.spriteHeight, 0, 0 + this.posY, this.oPanelsImgData.oData.spriteWidth, this.oPanelsImgData.oData.spriteHeight);
				for (var i = this.oScoreData.levelScore,
				s = 0; s < i.toString().length; s++) {
				...
				for (var i = this.oScoreData.highScore,
				s = 0; s < i.toString().length; s++) {
					t = parseFloat(i.toString().charAt(s));
...
function(e) {
	var t = function() {
		function e(e, t, n) {
			this.incX = 0,
			this.aSegs = new Array(0, 52, 87, 122, 157, 192),
			this.oImgData = e,
			this.canvasWidth = t,
			this.canvasHeight = n
		}
...
function(e) {
	var t = function() {
], [ - 25, -20], [ - 25, 0], [ - 25, 20], [ - 25, 40], [ - 25, 60], [ - 25, 80], [ - 25, 100], [ - 25, 120], [ - 5, -100], [ - 5, -80], [ - 5, -60], [ - 5, -40], [ - 5, -20], [ - 5, 0], [ - 5, 20], [ - 5, 40], [ - 5, 60], [ - 5, 80], [ - 5, 100], [ - 5, 120], [15, -100], [15, -80], [15, -60], [15, -40], [15, -20], [15, 0], [15, 20], [15, 40], [15, 60], [15, 80], [15, 100], [15, 120], [35, -40], [35, -20], [35, 0], [35, 20], [35, 40], [35, 60], [35, 80], [35, 100]]),
			this.oCrockeryImgData = e,
			this.oExpressionsImgData = t,
			this.oDirtImgData = n,
			this.crockeryId = i,
			this.crockeryCallback = r,
			this.canvasWidth = s,
			this.canvasHeight = o,
			...
			for (var u = 0; u < this.dirtNum; u++) {
				...
				this.aDirtPos.push(this.aDirtData[this.crockeryId][a]),
				this.aDirtData[this.crockeryId].splice(a, 1),
				this.aDirtPos[this.aDirtPos.length - 1].push((90 * Math.random() - 45) * (Math.PI / 180)),
				this.aDirtPos[this.aDirtPos.length - 1].push(Math.floor(1 * Math.random()))
			}
		...
		e.prototype.wash = function(e, t) {
			for (var n = 0,
			r = 0; r < this.aDirtPos.length; r++) {
				var i = t[0] - this.aDirtPos[r][0] - this.x,
				s = t[1] - this.aDirtPos[r][1] - this.y,
				o = i * i + s * s;
				5e3 > o && 10 != this.aDirtPos[r][3] && (this.aDirtPos[r][3] += e, this.aDirtPos[r][3] >= 4 && (this.aDirtPos[r][3] = 10, this.crockeryCallback("dirtClean"))),
				10 == this.aDirtPos[r][3] && n++
			}
		...
		e.prototype.renderDirty = function(e) {
			...
			for (var r = 0; r < this.aDirtPos.length; r++) if (Math.floor(this.aDirtPos[r][3]) < 4) {
				e.save(),
				e.translate(this.aDirtPos[r][0] + this.x, this.aDirtPos[r][1] + this.y + this.posY),
				e.rotate(this.aDirtPos[r][2]);
				...
				e.drawImage(this.oDirtImgData.img, t, n, this.oDirtImgData.oData.spriteWidth, this.oDirtImgData.oData.spriteHeight, -this.oDirtImgData.oData.spriteWidth / 2, -this.oDirtImgData.oData.spriteHeight / 2, this.oDirtImgData.oData.spriteWidth, this.oDirtImgData.oData.spriteHeight),
				e.restore()
			}
...
function(e) {
	var t = function(e) {
		...
		t.prototype.update = function(e) {
			...
			this.vx *= .9,
			this.x += this.vx * e,
			this.y > this.canvasHeight + 100 && (this.removeMe = !0)
		},
		t
	} (Utils.BasicSprite);
	e.Bubble = t
...
function() {
	...
	GameAPI.loadAPI(function(e) {
		console.log("GameAPI version " + e.version + " loaded!"),
		spilAPI = e,
		oSpilLogoData = spilAPI.Branding.getLogo(),
		oSpilMoreGamesData = spilAPI.Branding.getLink("more_games"),
		loadPreAssets()
	},
	e)
}),
...
water,
crockery,
crockeryId = Math.floor(5 * Math.random()),
aPrevWashPos,
aCurWashPos,
isWashing = !1,
bubbleLimiter,
